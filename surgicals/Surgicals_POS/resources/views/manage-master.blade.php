                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="nav navbar-nav ml-auto lw-top-menu">
                            ...
                            <li ui-sref-active="active" class="nav-item" ng-show="manageCtrl.isLoggedIn() && canAccess('manage.billing.write.store_bill') && canAccess('manage.billing.read.list')">
	                            <a class="nav-link lw-top-menu-link" title="<?=  __tr('Add New Bill')  ?>" ui-sref="billing_add"><i class="fa fa-plus" aria-hidden="true"></i> <?= __tr('Add New Bill') ?></a>
	                        </li>
	                        <li ui-sref-active="active" class="nav-item" ng-show="manageCtrl.isLoggedIn()">
	                         	<a class="nav-link lw-top-menu-link" ui-sref-active="active" ng-show="canAccess('manage.inventory.read.list')" title="<?=  __tr('Inventory')  ?>" ui-sref="inventory"><i class="fa fa-list-alt" aria-hidden="true"></i> <?= __tr('Inventory') ?></a>
	                        </li>
                            ...
                            <li class="nav-item dropdown" ng-show="manageCtrl.isLoggedIn()">
	                            ...
	                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
	                                <a class="dropdown-item" ui-sref-active="active" ui-sref="profile" title="<?=  __tr('Profile')  ?>"><?=  __tr('Profile')  ?></a>
	                                <a class="dropdown-item" ui-sref-active="active" ui-sref="changePassword" title="<?= __tr('Change Password') ?>"><?= __tr('Change Password') ?></a>
	                                <a class="dropdown-item" ui-sref-active="active" ng-if="manageCtrl.restrict_user_email_update == false && manageCtrl.auth_info.designation != 1" ui-sref="changeEmail" title="<?= __tr('Update Email') ?>"><?= __tr('Update Email') ?></a>
	                                <a class="dropdown-item" ui-sref-active="active" ng-if="manageCtrl.auth_info.designation == 1" ui-sref="changeEmail" title="<?= __tr('Update Email') ?>"><?= __tr('Update Email') ?></a>
	                                <a class="dropdown-item" href ng-click="manageCtrl.logoutUser()" title="<?= __tr('Logout') ?>"><?= __tr('Logout') ?> <i class="fa fa-sign-out"></i></a>
	                            </div>
	                        </li> 
                        </ul>
                    </div>
        ...
        <span class="text-muted">
                ...
                @if(getConfigurationSettings('enable_credit_info') == true)
                    <span class="pull-right">
                        <span> Powered by <strong>Inventario <small><?= config('lwSystem.version') ?></small></strong>   |  Designed &amp; Developed by <a href="http://livelyworks.net" title="Design &amp; Developed by livelyworks" target="_blank" >livelyworks</a></span>
                    </span>
                @endif
            </span>
        </footer>
